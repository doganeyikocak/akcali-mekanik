...
let db=null,currentConfig=loadCachedConfig(),selectedBlock="",selectedFloor="",selectedIssue="",selectedPhotoData="",syncRunning=false;

document.addEventListener("DOMContentLoaded",async()=>{
  document.getElementById("versionLabel").textContent="v"+APP_VERSION;
  await openDb();
  await requestPersistentStorage();
  bindNavigation();bindForm();bindButtons();loadSettings();renderChoices();
...
function showIosInstallHint(){
  ...
  if(isIos()&&!isStandalone()&&localStorage.getItem("hideIosInstall")!=="1")card.hidden=false;
}
...
function renderChoices(){
  renderChoiceGroup("blockChoices",currentConfig.blocks,v=>selectedBlock=v);
  renderChoiceGroup("floorChoices",currentConfig.floors,v=>selectedFloor=v);
  renderChoiceGroup("issueChoices",currentConfig.issues,v=>selectedIssue=v);
...
function renderChoiceGroup(id,items,setter){const el=document.getElementById(id);el.innerHTML="";(items||[]).forEach(item=>{const b=document.createElement("button");b.type="button";b.className="choice-btn";b.textContent=item;b.onclick=()=>{el.querySelectorAll(".choice-btn").forEach(x=>x.classList.remove("selected"));b.classList.add("selected");setter(item);};el.appendChild(b);});}
function fillSelect(id,items,placeholder){document.getElementById(id).innerHTML=`<option value="">${placeholder}</option>`+(items||[]).map(x=>`<option>${escapeHtml(x)}</option>`).join("");}

function bindForm(){
  document.getElementById("photoInput").addEventListener("change",async e=>{
    const f=e.target.files[0];if(!f)return;
    try{selectedPhotoData=await compressImage(f,1280,.68);const img=document.getElementById("photoPreview");img.src=selectedPhotoData;img.hidden=false;}
    catch(_){showToast("Fotoğraf işlenemedi. Tekrar çek.");}
  });

  document.getElementById("issueForm").addEventListener("submit",async e=>{
    e.preventDefault();
    if(!selectedBlock)return showToast("Blok seç.");
    if(!selectedFloor)return showToast("Kat seç.");
    if(!selectedIssue)return showToast("Hata türü seç.");
    if(!selectedPhotoData)return showToast("Fotoğraf çek.");

    const issue={
      localId:makeUuid(),createdAt:new Date().toISOString(),block:selectedBlock,floor:selectedFloor,
      location:document.getElementById("locationInput").value.trim(),issueType:selectedIssue,
      contractor:document.getElementById("contractorSelect").value,foreman:document.getElementById("foremanSelect").value,
      note:document.getElementById("noteInput").value.trim(),photoData:selectedPhotoData,
      deviceName:(localStorage.getItem("deviceName")||"").trim(),syncStatus:"pending",cloudId:"",lastError:""
    };
    if(!issue.location||!issue.contractor||!issue.foreman)return showToast("Mahal, taşeron ve formen alanlarını doldur.");

    try{
      await putIssue(issue);resetIssueForm();await refreshCounts();showView("homeView");
      showToast("✅ Kaydedildi. İnternet yoksa telefonda bekler.");
  ...
  if(!SR)voiceBtn.hidden=true;else voiceBtn.onclick=()=>{const r=new SR();r.lang="tr-TR";r.interimResults=false;r.onresult=e=>document.getElementById("noteInput").value=e.results[0][0].transcript;r.onerror=()=>showToast("Sesle yazma bu telefonda kullanılamadı.");r.start();};
}

function bindButtons(){
  document.getElementById("syncBtn").onclick=syncPending;
  document.getElementById("pendingSyncBtn").onclick=syncPending;
  document.getElementById("saveSettingsBtn").onclick=()=>{localStorage.setItem("deviceName",document.getElementById("deviceName").value.trim());localStorage.setItem("apiUrl",normalizeApiUrl(document.getElementById("apiUrl").value));showToast("Ayarlar kaydedildi.");};
...
async function refreshRemoteConfig(showResult=false){
  ...
  try{
    ...
    currentConfig=sanitizeConfig(data.config);localStorage.setItem("cachedConfig",JSON.stringify(currentConfig));renderChoices();
...
function sanitizeConfig(c){const clean=k=>Array.from(new Set((Array.isArray(c[k])?c[k]:DEFAULTS[k]).map(x=>String(x||"").trim()).filter(Boolean)));return {blocks:clean("blocks"),floors:clean("floors"),issues:clean("issues"),contractors:clean("contractors"),foremen:clean("foremen")};}

function resetIssueForm(){document.getElementById("issueForm").reset();selectedBlock=selectedFloor=selectedIssue=selectedPhotoData="";document.querySelectorAll(".choice-btn").forEach(x=>x.classList.remove("selected"));const img=document.getElementById("photoPreview");img.hidden=true;img.removeAttribute("src");}
function updateConnectionStatus(){const el=document.getElementById("connectionStatus");if(navigator.onLine){el.textContent="● İnternet var";el.className="status online";}else{el.textContent="● İnternet yok — kayıtlar telefonda";el.className="status offline";}}
...
function renderIssueCards(id,items){const el=document.getElementById(id);if(!items.length){el.innerHTML='<div class="card"><p>Kayıt yok.</p></div>';return;}el.innerHTML=items.map(x=>`<div class="card"><h3>${escapeHtml(x.block)} / ${escapeHtml(x.floor)} / ${escapeHtml(x.location)}</h3><p><b>Hata:</b> ${escapeHtml(x.issueType)}</p><p><b>Taşeron:</b> ${escapeHtml(x.contractor)}</p><p><b>Formen:</b> ${escapeHtml(x.foreman)}</p><p><b>Tarih:</b> ${new Date(x.createdAt).toLocaleString("tr-TR")}</p>${x.cloudId?`<p><b>Kayıt No:</b> ${escapeHtml(x.cloudId)}</p>`:""}${x.note?`<p><b>Not:</b> ${escapeHtml(x.note)}</p>`:""}${x.lastError?`<p><b>Son hata:</b> ${escapeHtml(x.lastError)}</p>`:""}<span class="tag ${x.syncStatus==="synced"?"synced":"pending"}">${x.syncStatus==="synced"?"Gönderildi":"Gönderilmeyi bekliyor"}</span>${x.photoData?`<img src="${x.photoData}" alt="Hata fotoğrafı">`:""}</div>`).join("");}
...
async function syncPending(){
  ...
  const all=await getAllIssues(),pending=all.filter(x=>x.syncStatus!=="synced");if(!pending.length)return showToast("Gönderilecek kayıt yok.");
  ...
  try{
    for(const issue of pending){
      try{
        ...
        const data=await res.json();if(!data.ok)throw new Error(data.error||"Sunucu hatası");
        issue.syncStatus="synced";issue.cloudId=data.recordId||issue.cloudId||"";issue.lastError="";await putIssue(issue);ok++;
