...
function doPost(e){
  ...
  try{
    ...
    const ss=spreadsheet_();const sheet=ensureTrackingSheet_(ss);
    ...
    const recordId=createUniqueId_();const photoUrl=savePhoto_(recordId,issue.photoData);
    ...
    sheet.getRange(row,1).setNumberFormat("@");sheet.getRange(row,2).setNumberFormat("dd.MM.yyyy HH:mm");
...
function getConfig_(){
  const ss=spreadsheet_();const s=ensureDefinitionsSheet_(ss);
  const rows=Math.max(1,s.getLastRow()-3);const cols=Math.max(9,s.getMaxColumns());
...
function ensureDefinitionsSheet_(ss){
  ...
  if(!s){s=ss.insertSheet(DEFINITIONS_SHEET);s.getRange(3,1,1,9).setValues([["BLOKLAR","KATLAR","SİSTEMLER","ÖNCELİKLER","DURUMLAR","TAŞERONLAR","FORMENLER","MEKANİK OFİS","HATA TÜRLERİ"]]);}
  else{if(s.getMaxColumns()<9)s.insertColumnsAfter(s.getMaxColumns(),9-s.getMaxColumns());if(!s.getRange(3,9).getValue())s.getRange(3,9).setValue("HATA TÜRLERİ");}
  return s;
}
function ensureTrackingSheet_(ss){
  ...
  }else if(s.getMaxColumns()<25){s.insertColumnsAfter(s.getMaxColumns(),25-s.getMaxColumns());s.getRange(4,25).setValue("PWA Yerel ID");}
  else if(!s.getRange(4,25).getValue())s.getRange(4,25).setValue("PWA Yerel ID");
  return s;
}
